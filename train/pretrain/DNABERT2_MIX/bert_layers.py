...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        if subset_mask is None:
            if mix:
                if mix_layer_num==-1:
                    all_len = len(self.layer)
                    mix_layer_num = random.randint(0,all_len-1)
                for layer_module in self.layer[:mix_layer_num]:
                ...
                hidden_states, hidden_rand, hidden_lam = mixup(input=hidden_states, alpha=mix_alpha)
                
                #modify attention mask
                attention_mask = attention_mask * attention_mask[hidden_rand]
                attention_mask_bool = attention_mask.bool()
        ...
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        if mix:
            return all_encoder_layers, hidden_rand, hidden_lam, attention_mask
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        if mix:    
            encoder_outputs, hidden_rand, hidden_lam, attention_mask = self.encoder(
                embedding_output,
                attention_mask,
        ...
        if mix:
            return (encoder_outputs, None), hidden_rand, hidden_lam, attention_mask
        if self.pooler is not None:
